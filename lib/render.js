...
// Cache with 1 day expiry.
let cache = new Lru({ 'max': +Infinity, 'maxAge': 1e3 * 60 * 60 * 24 });

module.exports = (opts, files, url, cb) => {
  ...
  as.forEachOf(obj.data.helpers, (fn, key, cb) => {
    ...
    cons.swig(`${opts.SITE}layouts/${obj.data.template}`, _.extend(obj.data, {
